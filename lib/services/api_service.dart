...
class ApiService {
  ...
  static Future<Map<String, dynamic>> bloquearUsuario({
    required int userId,
  }) async {
    final token = await AuthService.obtenerToken();
    ...
    final response = await http
        .post(
          Uri.parse(
            '$baseUrl/mobile/users/$userId/block/',
          ),
          headers: {
            'Authorization': 'Token $token',
            'Accept': 'application/json',
    ...
    if (response.statusCode != 200) {
      ...
      throw Exception(
        'No pudimos bloquear a este usuario.',
      );
    }

    if (decodedData == null) {
      throw Exception(
        'La respuesta del bloqueo no es válida.',
      );
    }

    return decodedData;
  }
  static Future<Map<String, dynamic>> reportarHuella({
    required int whisperId,
    required String reason,
    String? message,
  }) async {
    final token = await AuthService.obtenerToken();
    ...
    final response = await http
        .post(
          Uri.parse(
            '$baseUrl/mobile/whispers/$whisperId/report/',
          ),
          headers: {
            'Authorization': 'Token $token',
            'Accept': 'application/json',
